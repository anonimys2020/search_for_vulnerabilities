...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Пример: /search ios 1 1')
    ...
    try:
        ...
        if len(args) >= 3:
            ...
            if page >= 1 and size >= 1:
                q = query(message.text) + '+уязвимость' # добавляем слово уязвимость для того, чтобы выводило только уязвимости
